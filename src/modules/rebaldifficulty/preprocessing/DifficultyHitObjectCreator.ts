...
export class DifficultyHitObjectCreator {
    ...
    generateDifficultyObjects(params: {
        ...
        const scalingFactor: number = this.getScalingFactor(this.objects[0].radius);
        ...
        for (let i = 0; i < this.objects.length; ++i) {
            ...
            if (lastObject.object instanceof Slider) {
                ...
                object.travelTime = Math.max(lastObject.object.lazyTravelTime / params.speedMultiplier, this.minDeltaTime);
                object.movementTime = Math.max(object.strainTime - object.travelTime, this.minDeltaTime);

                // Jump distance from the slider tail to the next object, as opposed to the lazy position of JumpDistance.
                const tailJumpDistance: number = lastObject.object.tailCircle.stackedPosition.subtract(object.object.stackedPosition).length * scalingFactor;
                ...
                object.movementDistance = Math.max(0, Math.min(object.jumpDistance - (this.maximumSliderRadius - this.assumedSliderRadius), tailJumpDistance - this.maximumSliderRadius));
            } else {
    ...
    private calculateSliderCursorPosition(slider: Slider): void {
        ...
        slider.lazyTravelTime = slider.nestedHitObjects[slider.nestedHitObjects.length - 1].startTime - slider.startTime;
        ...
        for (let i = 1; i < slider.nestedHitObjects.length; ++i) {
            ...
            let currentMovement: Vector2 = currentMovementObject.stackedPosition.subtract(currentCursorPosition);
            let currentMovementLength: number = scalingFactor * currentMovement.length;
            ...
            if (i === slider.nestedHitObjects.length - 1) {
                ...
                const lazyMovement: Vector2 = slider.lazyEndPosition.subtract(currentCursorPosition);
        ...
        if (this.mode === modes.droid) {
            slider.lazyTravelDistance *= Math.pow(1 + (slider.repetitions - 1) / 4, 1 / 4);
        } else {
            slider.lazyTravelDistance *= Math.pow(1 + (slider.repetitions - 1) / 2.5, 1 / 2.5);
        }
    }
