...
export class PrototypePPCollectionManager extends DatabaseCollectionManager<DatabasePrototypePP, PrototypePP> {
    protected override readonly utilityInstance: DatabaseUtilityConstructor<DatabasePrototypePP, PrototypePP>;
     ...
     */
    async searchPlayers(page: number, searchQuery?: string): Promise<PrototypePP[]> {
        ...
        const userBind: DatabasePrototypePP[] = await this.collection.find(
            query, { projection: { _id: 0, uid: 1, pptotal: 1 , prevpptotal: 1, username: 1 } }
    ...
    getFromUid(uid: number): Promise<PrototypePP | null> {
        return this.getOne({ previous_bind: { $all: [ uid ] } }, { projection: { _id: 0, uid: 1, username: 1, pp: 1, pptotal: 1, prevpptotal: 1 } });
    }
}