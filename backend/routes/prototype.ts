...
router.get(["/", "/leaderboard"], async (req, res) => {
    ...
    const result: PrototypePP[] =
        await DatabaseManager.aliceDb.collections.prototypePP.searchPlayers(
            page,
            searchQuery
        );
...
router.get("/profile", async (req, res) => {
    ...
    const playerInfo: PrototypePP | null =
        await DatabaseManager.aliceDb.collections.prototypePP.getFromUid(uid);
    ...
    res.render(renderPath, {
        ...
        pprank: await DatabaseManager.aliceDb.collections.prototypePP.getUserDPPRank(
            playerInfo.pptotal
        ),
        prevpprank:
            await DatabaseManager.elainaDb.collections.userBind.getUserDPPRank(
                playerInfo.prevpptotal
            ),
...
router.get("/calculate", (_, res) => {
    res.render(
        join(Util.getFrontendPath(), "render", "prototype", "calculate")
    );
});
