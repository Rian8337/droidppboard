...
export class Chart implements ChartInitializer {
    ...
    private readonly font: string = "bold 12pt Exo";
    private readonly axisLabelFont: string = "bold 11pt Exo";
    ...
    private timeString(second: number): string {
        const str: string = new Date(1000 * Math.ceil(second)).toISOString();

        return second < 3600 ? str.substr(14, 5) : str.substr(11, 8);
    }
}