...
import { PrototypePP } from "app-database";
import { LeaderboardSetting } from "../interfaces/LeaderboardSetting";

const PrototypeLeaderboardNavigator: Context<LeaderboardSetting<PrototypePP>> =
    createContext<LeaderboardSetting<PrototypePP>>({
        data: [],
        page: 1,
        query: "",
        isSearchReady: false,
...
export const PrototypeLeaderboardNavigatorProvider: FC = (props) => {
    const [data, setData] = useState<PrototypePP[]>([]);
    ...
    const modifyData = (data: PrototypePP[] = []) => {
        setData(data);
    };
