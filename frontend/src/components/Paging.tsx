...
import "../styles/modules/components/Paging.module.css";
...
export default function Paging<T extends PagingSetting>(props: { state: T }) {
    const checkPageValidity = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!isNaN(newPage)) {
            props.state.setPage(Math.max(newPage, 1));
        }
    };
    ...
    return (
        <div className="nav-button">
            ...
            <button
                className="page-nav"
                disabled={!props.state.enablePaging || props.state.page === 1}
                onClick={(event) => {
                    event.preventDefault();
                    ...
                    props.state.setPage(props.state.page - 1);
                }}
            >
                &lt;&lt;&lt; Previous Page
            </button>
